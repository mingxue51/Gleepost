...
//    [self.userName setText:incomingPost.user.name];
    ...
    //TODO: Dynamically relocate the elements below the content label if there is a need.
}

/**
 Called only when the post not contains image.
 
 TODO: Changes
 size of view: 156
 line: 20,99    280,1
 information label: 20,101  218,21
 buttons: 20,126    49,22 (like button)
 */
-(void)enableTextPost
{
    NSLog(@"enableTextPost");
    CGRect viewFrame = self.frame;
    [self setFrame:CGRectMake(viewFrame.origin.x, viewFrame.origin.y, viewFrame.size.width, 156)];
}
